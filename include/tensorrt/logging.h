...
public:
    ...
    void putOutput()
    {
        if (mShouldLog)
        {
            // Apply color based on severity (extracted from prefix)
            const char *color = AnsiColor::RESET;
            if (mPrefix == "[F] ")
                color = AnsiColor::BOLD_RED;
            else if (mPrefix == "[E] ")
                color = AnsiColor::RED;
            else if (mPrefix == "[W] ")
